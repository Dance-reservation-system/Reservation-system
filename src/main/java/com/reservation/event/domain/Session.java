...
import com.reservation.event.domain.exception.ReservationsExistException;
import com.reservation.event.domain.exception.SessionAlreadyCancelledException;
import com.reservation.event.domain.exception.SessionUpdatedException;
...
public class Session implements AggregateRoot<SessionEvent> {
    ...
    public void updateDetails(InstructorId newInstructorId, HallId newHallId, SessionTitle newTitle, SessionCapacity newCapacity, RecurrencePattern newRecurrence) {
        if(!status.isScheduled()){
            throw new SessionUpdatedException();
        }
        this.instructorId = Objects.requireNonNull(newInstructorId);
        ...
        registerEvent(new SessionUpdated(sessionId));
    }
